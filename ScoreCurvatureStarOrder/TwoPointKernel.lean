...
theorem twoPointKernel_nonneg_of_log_ratio {S Sprime : ℝ → ℝ} {a x t : ℝ}
    (hx : S (a + x) ≠ 0)
    (hr : 0 < S (a + t) / S (a + x))
    (hlog :
      Real.log (S (a + t) / S (a + x)) ≤
        (Sprime (a + x) / S (a + x)) * (t - x)) :
  ...
  have hmul :
      r * Real.log r ≤
        r * ((Sprime (a + x) / S (a + x)) * (t - x)) := by
    apply mul_le_mul_of_nonneg_left
    · simpa [r] using hlog
    · exact (by simpa [r] using hr.le)
  have hnorm :
      r - 1 ≤ r * ((Sprime (a + x) / S (a + x)) * (t - x)) :=
    hratio.trans hmul
  have hnonneg :
      0 ≤ (S (a + x)) ^ 2 *
        (r * ((Sprime (a + x) / S (a + x)) * (t - x)) - (r - 1)) := by
    exact mul_nonneg (sq_nonneg _) (sub_nonneg.mpr hnorm)
  have hid :
      (S (a + x)) ^ 2 *
          (r * ((Sprime (a + x) / S (a + x)) * (t - x)) - (r - 1)) =
        twoPointKernel S Sprime a x t := by
    dsimp [r, twoPointKernel]
    field_simp [hx]
  rw [← hid]
  exact hnonneg

theorem twoPointKernel_nonneg_of_right_zero {S Sprime : ℝ → ℝ} {a x t : ℝ}
    (ht : S (a + t) = 0) :
    0 ≤ twoPointKernel S Sprime a x t := by
  simpa [twoPointKernel, ht, pow_two] using sq_nonneg (S (a + x))

theorem twoPointKernel_nonneg_of_left_zero {S Sprime : ℝ → ℝ} {a x t : ℝ}
    (ha : 0 ≤ a) (hx0 : 0 ≤ x) (ht0 : 0 ≤ t)
    (hmono : MonotoneOn S (Set.Ici 0))
    (hSprime : 0 ≤ Sprime (a + x))
    (hx : S (a + x) = 0) :
    0 ≤ twoPointKernel S Sprime a x t := by
  ...
  have hmain : 0 ≤ Sprime (a + x) * ((t - x) * S (a + t)) :=
    mul_nonneg hSprime hprod
  simpa [twoPointKernel, hx, mul_assoc] using hmain

theorem twoPointKernel_nonneg_of_left_nonpos_right_nonneg
    {S Sprime : ℝ → ℝ} {a x t : ℝ}
    (hSprime : 0 ≤ Sprime (a + x))
    (hxt : x ≤ t)
    (hsx : S (a + x) ≤ 0)
    (hst : 0 ≤ S (a + t)) :
    0 ≤ twoPointKernel S Sprime a x t := by
  have hfirst : 0 ≤ Sprime (a + x) * (t - x) * S (a + t) :=
    mul_nonneg (mul_nonneg hSprime (sub_nonneg.mpr hxt)) hst
...
theorem twoPointKernel_nonneg_of_left_nonneg_right_nonpos
    ...
    (hst : S (a + t) ≤ 0) :
    0 ≤ twoPointKernel S Sprime a x t := by
  have hpair : Sprime (a + x) * (t - x) ≤ 0 :=
    mul_nonpos_of_nonneg_of_nonpos hSprime (sub_nonpos.mpr htx)
  have hfirst : 0 ≤ Sprime (a + x) * (t - x) * S (a + t) :=
