...
absolutely integrable on the positive half-line. -/
theorem powerWeightedUnshifted_score_integrableOn_Ioi
  ...
  · have hp' : -1 < p - 1 := by linarith
    have h := powerWeightedShift_score_integrableOn_Ioi
  ...
  · have hp_nonpos : p ≤ 0 := le_of_not_gt hp0
    ...
    · rw [← intervalIntegrable_iff_integrableOn_Ioc_of_le hBpos.le]
      have hScont : ContinuousOn S [[0, B]] := by
        intro x hx
        rw [uIcc_of_le hBpos.le] at hx
        exact (hS x hx.1).continuousAt.continuousWithinAt
      have hthetacont : ContinuousOn theta [[0, B]] := by
        intro x hx
        rw [uIcc_of_le hBpos.le] at hx
        exact (htheta_deriv x hx.1).continuousAt.continuousWithinAt
      have hlocal :=
    ...
    · have htheta_zero : Tendsto theta atTop (𝓝 0) := by
        have hboundary := powerWeightedShift_boundary_atTop
      ...
      have hderivB :
        ...
        intro x hx
        have hx0 : 0 ≤ x := hR.trans (hRB.trans hx)
        exact htheta_deriv x hx0
      ...
      have hScont : ContinuousOn S (Set.Ioi B) := by
        intro x hx
        have hx0 : 0 ≤ x := (hBpos.trans hx).le
        exact (hS x hx0).continuousAt.continuousWithinAt
      have hthetacont : ContinuousOn theta (Set.Ioi B) := by
        intro x hx
        have hx0 : 0 ≤ x := (hBpos.trans hx).le
        exact (htheta_deriv x hx0).continuousAt.continuousWithinAt
...
/-- On a sufficiently late tail, the absolute shifted `a`-derivative of `theta(a+x)`
is bounded by the unshifted score weight, uniformly for every `a ≥ 0`. -/
theorem exists_shifted_thetaDeriv_tail_majorant
  ...
  rcases automatic_score_mul_theta_antitoneOn_tail
