...
/-- Joint compact-parameter domination for the second logarithmic power moment.

For every compact rectangle `p ∈ [p₀,p₁]`, `a ∈ [0,A]` with `p₀ > -1`,
there is a single integrable function on `(0,∞)` dominating

`x^p * (log x)^2 * theta(a+x)`

simultaneously for all parameters in the rectangle.  As in the first-log
majorant, the lower exponent is the worst case on `(0,1]` and the upper
exponent is the worst case on `[1,∞)`.  On bounded spatial intervals the shift
is absorbed by a compact positive ratio bound for `theta`; on the tail the
positive-score estimate gives `theta(a+x) ≤ theta(x)` for every `a ≥ 0`.
The endpoint majorants are integrable by the verified second logarithmic moment
theorem. -/
theorem exists_powerWeightedShift_log_sq_compact_majorant_within
    ...
    ∃ bound : ℝ → ℝ,
      ...
      ∀ p ∈ Set.Icc p₀ p₁, ∀ a ∈ Set.Icc (0 : ℝ) A, ∀ x ∈ Set.Ioi (0 : ℝ),
        ‖x ^ p * (Real.log x) ^ 2 * theta (a + x)‖ ≤ bound x := by
  ...
  have hlogsq₀ := powerWeightedShift_log_sq_integrableOn_Ioi_within
  ...
  have hnorm₀ :
      IntegrableOn
        (fun x : ℝ => ‖x ^ p₀ * (Real.log x) ^ 2 * theta x‖)
        (Set.Ioi (0 : ℝ)) := by
    change Integrable
      (fun x : ℝ => ‖x ^ p₀ * (Real.log x) ^ 2 * theta x‖)
      (volume.restrict (Set.Ioi (0 : ℝ)))
    simpa using hlogsq₀.norm
  have hnorm₁ :
      IntegrableOn
        (fun x : ℝ => ‖x ^ p₁ * (Real.log x) ^ 2 * theta x‖)
        (Set.Ioi (0 : ℝ)) := by
  ...
  let bound : ℝ → ℝ := fun x =>
    if x ≤ 1 then D * ‖x ^ p₀ * (Real.log x) ^ 2 * theta x‖
    else if x ≤ B then D * ‖x ^ p₁ * (Real.log x) ^ 2 * theta x‖
    else ‖x ^ p₁ * (Real.log x) ^ 2 * theta x‖
  ...
  have hlogsq_nonneg : 0 ≤ (Real.log x) ^ 2 := sq_nonneg _

  have hnorm_shift :
      ‖x ^ p * (Real.log x) ^ 2 * theta (a + x)‖ =
        x ^ p * (Real.log x) ^ 2 * theta (a + x) := by
    rw [Real.norm_eq_abs, abs_mul, abs_mul,
      abs_of_nonneg (Real.rpow_nonneg hxpos.le p), abs_of_nonneg hlogsq_nonneg,
      abs_of_pos htheta_shift_pos]
  rw [hnorm_shift]
  ...
  · have hpow_le : x ^ p ≤ x ^ p₀ :=
    ...
    have hpowlogsq_le :
        x ^ p * (Real.log x) ^ 2 ≤ x ^ p₀ * (Real.log x) ^ 2 :=
      mul_le_mul_of_nonneg_right hpow_le hlogsq_nonneg
    have hleft_nonneg : 0 ≤ x ^ p₀ * (Real.log x) ^ 2 :=
      mul_nonneg (Real.rpow_nonneg hxpos.le p₀) hlogsq_nonneg
    have hnorm₀x :
        ‖x ^ p₀ * (Real.log x) ^ 2 * theta x‖ =
          x ^ p₀ * (Real.log x) ^ 2 * theta x := by
      rw [Real.norm_eq_abs, abs_mul, abs_mul,
        abs_of_nonneg (Real.rpow_nonneg hxpos.le p₀), abs_of_nonneg hlogsq_nonneg,
        abs_of_pos htheta_x_pos]
    calc
      x ^ p * (Real.log x) ^ 2 * theta (a + x) ≤
          x ^ p₀ * (Real.log x) ^ 2 * theta (a + x) :=
        mul_le_mul_of_nonneg_right hpowlogsq_le htheta_shift_pos.le
      _ ≤ x ^ p₀ * (Real.log x) ^ 2 * (D * theta x) :=
        mul_le_mul_of_nonneg_left htheta_ratio hleft_nonneg
      _ = D * ‖x ^ p₀ * (Real.log x) ^ 2 * theta x‖ := by
        rw [hnorm₀x]
        ring
    ...
    · have hBx : B < x := lt_of_not_ge hxB
      ...
      calc
        x ^ p * (Real.log x) ^ 2 * theta (a + x) ≤
            x ^ p₁ * (Real.log x) ^ 2 * theta (a + x) :=
          mul_le_mul_of_nonneg_right hpowlogsq_le htheta_shift_pos.le
        _ ≤ x ^ p₁ * (Real.log x) ^ 2 * theta x :=
          mul_le_mul_of_nonneg_left htheta_shift_le hleft_nonneg
        _ = ‖x ^ p₁ * (Real.log x) ^ 2 * theta x‖ := hnorm₁x.symm
        _ = bound x := by simp [bound, hx1, hxB]
